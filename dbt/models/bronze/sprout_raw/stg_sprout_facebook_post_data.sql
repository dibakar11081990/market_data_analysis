...
-- MODEL  : stg_sprout_facebook_post_data
-- SOURCE : {{ source('sprout_raw', 'FACEBOOK_POST_DATA') }}
-- NOTES  : Replaces var('SPROUT.FACEBOOK_POST_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'FACEBOOK_POST_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'FACEBOOK_POST_DATA') }}
    {% endif %}
