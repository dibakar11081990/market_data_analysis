...
-- MODEL  : stg_sprout_youtube_profile_data
-- SOURCE : {{ source('sprout_raw', 'YOUTUBE_PROFILE_DATA') }}
-- NOTES  : Replaces var('SPROUT.YOUTUBE_PROFILE_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'YOUTUBE_PROFILE_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'YOUTUBE_PROFILE_DATA') }}
    {% endif %}
