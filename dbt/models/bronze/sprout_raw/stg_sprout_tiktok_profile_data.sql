...
-- MODEL  : stg_sprout_tiktok_profile_data
-- SOURCE : {{ source('sprout_raw', 'TIKTOK_PROFILE_DATA') }}
-- NOTES  : Replaces var('SPROUT.TIKTOK_PROFILE_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'TIKTOK_PROFILE_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'TIKTOK_PROFILE_DATA') }}
    {% endif %}
