...
-- MODEL  : stg_sprout_facebook_profile_data
-- SOURCE : {{ source('sprout_raw', 'FACEBOOK_PROFILE_DATA') }}
-- NOTES  : Replaces var('SPROUT.FACEBOOK_PROFILE_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'FACEBOOK_PROFILE_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'FACEBOOK_PROFILE_DATA') }}
    {% endif %}
