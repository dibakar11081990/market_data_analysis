...
-- MODEL  : stg_sprout_linkedin_post_data
-- SOURCE : {{ source('sprout_raw', 'LINKEDIN_POST_DATA') }}
-- NOTES  : Replaces var('SPROUT.LINKEDIN_POST_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'LINKEDIN_POST_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'LINKEDIN_POST_DATA') }}
    {% endif %}
