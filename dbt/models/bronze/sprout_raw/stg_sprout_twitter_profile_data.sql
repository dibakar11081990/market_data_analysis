...
-- MODEL  : stg_sprout_twitter_profile_data
-- SOURCE : {{ source('sprout_raw', 'TWITTER_PROFILE_DATA') }}
-- NOTES  : Replaces var('SPROUT.TWITTER_PROFILE_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'TWITTER_PROFILE_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'TWITTER_PROFILE_DATA') }}
    {% endif %}
