...
-- MODEL  : stg_sprout_linkedin_profile_data
-- SOURCE : {{ source('sprout_raw', 'LINKEDIN_PROFILE_DATA') }}
-- NOTES  : Replaces var('SPROUT.LINKEDIN_PROFILE_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'LINKEDIN_PROFILE_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'LINKEDIN_PROFILE_DATA') }}
    {% endif %}
