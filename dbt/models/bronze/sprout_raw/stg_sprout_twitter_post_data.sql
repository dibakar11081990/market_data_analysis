...
-- MODEL  : stg_sprout_twitter_post_data
-- SOURCE : {{ source('sprout_raw', 'TWITTER_POST_DATA') }}
-- NOTES  : Replaces var('SPROUT.TWITTER_POST_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'TWITTER_POST_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'TWITTER_POST_DATA') }}
    {% endif %}
