...
-- MODEL  : stg_sprout_tiktok_post_data
-- SOURCE : {{ source('sprout_raw', 'TIKTOK_POST_DATA') }}
-- NOTES  : Replaces var('SPROUT.TIKTOK_POST_DATA')
...
FROM
    {% if target.name | lower == 'prd' %}
        {{ source('sprout_raw', 'TIKTOK_POST_DATA') }}
    {% else %}
        {{ source('sprout_raw_dev', 'TIKTOK_POST_DATA') }}
    {% endif %}
