'''
This python script picks and loads Instagram POST Data from SPROUT API creates a equivalent csv file and puts it to S3
...
if os.environ['ENVIRONMENT'] == 'prd':
    ...
    key =  'test/dt='+date+'/instagram_post_data.csv' 
...
print("Instagram Data extraction started...")
...
def get_snowflake_data():
    ...
    try:
        ...
        sql = f"""
            ...
            WHERE NETWORK_TYPE IN ('fb_instagram_account')
        """
...
col_list = [
  ...
  'hashtags',
  'metrics.lifetime.link_in_bio_clicks',
  ...
  'metrics.lifetime.saves',
  'metrics.lifetime.story_exits',
  'metrics.lifetime.story_taps_back',
  'metrics.lifetime.story_taps_forward',
  'internal.tags',
  "perma_link",
]
...
post_df.rename(columns={
  ...
  'metrics.lifetime.link_in_bio_clicks': 'link_in_bio_clicks',
  ...
  'metrics.lifetime.saves': 'saves',
  'metrics.lifetime.story_exits': 'story_exits',
  'metrics.lifetime.story_taps_back': 'story_taps_back',
  'metrics.lifetime.story_taps_forward': 'story_taps_forward',
...
# doing the truncate and load of the target table in SF
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{snowflake_schema}"."{TARGET_TABLE}"("INGESTION_TIMESTAMP", "CREATED_TIME", "CUSTOMER_PROFILE_ID", "POST_TYPE", "NETWORK", "LONG_URL", "SHORT_URL", "HASHTAGS", "LINK_IN_BIO_CLICKS", "REACTIONS", "SHARES_COUNT", "LIKES", "COMMENTS_COUNT", "VIDEO_VIEWS", "IMPRESSIONS", "IMPRESSIONS_UNIQUE", "SAVES", "STORY_EXITS", "STORY_TAPS_BACK", "STORY_TAPS_FORWARD", "TAG_ID", "POST_URL", "TAGS")
        FROM 
                's3://{bucket}/'
    ...
    return '\n:::END OF Customer POST Data For Instagram Extract Load script:::'    
