'''
This python script picks and loads TIKTOK POST Data from SPROUT API creates a equivalent csv file and puts it to S3
...
if os.environ['ENVIRONMENT'] == 'prd':
    key =  'sprout/dt='+date+'/tiktok_post_data.csv' 
    tags_key = 'sprout/dt='+date+'/customer_tags_data.csv'

else:
    key =  'test/dt='+date+'/tiktok_post_data.csv' 
...
col_list = [
  ...
  'metrics.lifetime.video_views_p100_per_view',
  'metrics.video_length',
  'internal.tags',
  "perma_link",
]
...
post_df.rename(columns={
  ...
  'metrics.lifetime.video_views_p100_per_view': 'video_views_p100_per_view',
...
# doing the truncate and load of the target table in SF
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{snowflake_schema}"."{TARGET_TABLE}"("INGESTION_TIMESTAMP", "CREATED_TIME", "CUSTOMER_PROFILE_ID", "POST_TYPE", "NETWORK", "LONG_URL", "SHORT_URL", "HASHTAGS", "POST_CONTENT_CLICKS", "REACTIONS", "SHARES_COUNT", "LIKES", "COMMENTS_COUNT", "VIDEO_VIEWS", "VIDEO_VIEW_TIME", "VIDEO_VIEW_TIME_PER_VIEW", "VIDEO_VIEWS_P100_PER_VIEW", "VIDEO_LENGTH", "TAG_ID", "POST_URL", "TAGS")
        FROM 
                's3://{bucket}/'
    ...
    return '\n:::END OF Customer POST Data For TIKTOK Extract Load script:::'   
