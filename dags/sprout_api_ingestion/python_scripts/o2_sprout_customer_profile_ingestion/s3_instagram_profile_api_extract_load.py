'''
This python script picks and loads Instagram Profile Data from SPROUT API creates a equivalent csv file and puts it to S3
...
TARGET_TABLE = 'INSTAGRAM_PROFILE_DATA'

bucket = 'com.tredence_analytics.marketing.mars.prd.ue1.matillion-data'
#Dynamically setting the S3 key or prefix
if os.environ['ENVIRONMENT'] == 'prd':
    key =  'sprout/dt='+date+'/instagram_profile_data.csv'
else:
    key =  'test/dt='+date+'/instagram_profile_data.csv'  
...
profile_df.rename(columns={
  ...
  'metrics.profile_views_unique': 'profile_views_unique',
  'metrics.website_clicks': 'website_clicks',
...
# doing the truncate and load of the target table in SF
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{snowflake_schema}"."{TARGET_TABLE}"("CUSTOMER_PROFILE_ID", "REPORTING_PERIOD_DAY", "INGESTION_TIMESTAMP", "COMMENTS_COUNT", "ENGAGEMENTS_OTHER", "IMPRESSIONS", "LIFETIME_SNAPSHOT_FOLLOWERS_COUNT", "LIKES", "NET_FOLLOWER_GROWTH", "POST_APP_ENGAGEMENTS", "POST_APP_INSTALLS", "POST_APP_OPENS", "POST_CONTENT_CLICKS", "POST_CONTENT_CLICKS_OTHER", "POST_DETAIL_EXPAND_CLICKS", "POST_HASHTAG_CLICKS", "POST_LINK_CLICKS", "POST_MEDIA_CLICKS", "POST_MEDIA_VIEWS", "POST_PROFILE_CLICKS", "POSTS_SENT_COUNT", "REACTIONS", "SHARES_COUNT", "VIDEO_VIEWS", "IMPRESSIONS_UNIQUE", "PROFILE_VIEWS_UNIQUE", "WEBSITE_CLICKS", "POSTS_SENT_BY_POST_TYPE")
        FROM 
                's3://{bucket}/'
    ...
    return '\n:::END OF Customer Profile Data For Instagram Extract Load script:::'   



#loading Instagram data to snowflake using copyinto
print(sf_load())
