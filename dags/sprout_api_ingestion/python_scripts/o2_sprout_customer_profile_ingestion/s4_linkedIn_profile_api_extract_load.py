'''
This python script picks and loads Linkdein Profile Data from SPROUT API creates a equivalent csv file and puts it to S3
...
TARGET_TABLE = 'LINKEDIN_PROFILE_DATA'

bucket = 'com.tredence_analytics.marketing.mars.prd.ue1.matillion-data'
#Dynamically setting the S3 key or prefix
if os.environ['ENVIRONMENT'] == 'prd':
    key =  'sprout/dt='+date+'/linkedin_company_profile_data.csv' 
else:
    key =  'test/dt='+date+'/linkedin_company_profile_data.csv'
...
col_list = [
	...
	'metrics.lifetime_snapshot.followers_by_job_function',
	'metrics.lifetime_snapshot.followers_by_seniority',
...
for cust_id in sf_data:
  ...
  payload = {
    ...
    "metrics": [
      ...
      "lifetime_snapshot.followers_by_job_function",
      "lifetime_snapshot.followers_by_seniority",
      "net_follower_growth",
      "followers_gained",
...
profile_df.rename(columns={
	...
	'metrics.lifetime_snapshot.followers_by_job_function': 'followers_by_job_function',
	'metrics.lifetime_snapshot.followers_by_seniority': 'followers_by_seniority',
...
# doing the truncate and load of the target table in SF
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{snowflake_schema}"."{TARGET_TABLE}"("CUSTOMER_PROFILE_ID", "REPORTING_PERIOD_DAY", "INGESTION_TIMESTAMP", "FOLLOWERS_COUNT", "FOLLOWERS_BY_JOB_FUNCTION", "FOLLOWERS_BY_SENIORITY", "NET_FOLLOWER_GROWTH", "FOLLOWERS_GAINED", "FOLLOWERS_GAINED_ORGANIC", "FOLLOWERS_GAINED_PAID", "FOLLOWERS_LOST", "IMPRESSIONS", "IMPRESSIONS_UNIQUE", "REACTIONS", "COMMENTS_COUNT", "SHARES_COUNT", "POST_CONTENT_CLICKS", "POSTS_SENT_COUNT", "POSTS_SENT_BY_POST_TYPE", "POSTS_SENT_BY_CONTENT_TYPE")
        FROM 
                's3://{bucket}/'
    ...
    return '\n:::END OF Customer Profile Data For Linkedin Extract Load script:::' 



#loading Linkedin data to snowflake using copyinto
print(sf_load())
