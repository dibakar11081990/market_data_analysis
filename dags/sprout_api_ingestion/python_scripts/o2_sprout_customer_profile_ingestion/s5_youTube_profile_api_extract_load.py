'''
This python script picks and loads Youtube Profile Data from SPROUT API creates a equivalent csv file and puts it to S3
...
TARGET_TABLE = 'YOUTUBE_PROFILE_DATA' 

bucket = 'com.tata.marketing.mars.prd.ue1.matillion-data'
#Dynamically setting the S3 key or prefix
if os.environ['ENVIRONMENT'] == 'prd':
    key =  'sprout/dt='+date+'/youtube_profile_data.csv' 
else:
    key =  'test/dt='+date+'/youtube_profile_data.csv'
...
# doing the truncate and load of the target table in SF
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{snowflake_schema}"."{TARGET_TABLE}"("CUSTOMER_PROFILE_ID", "REPORTING_PERIOD_DAY", "INGESTION_TIMESTAMP", "FOLLOWERS_COUNT", "NET_FOLLOWER_GROWTH", "FOLLOWERS_GAINED", "FOLLOWERS_LOST", "POSTS_SENT_COUNT") 
        FROM 
                's3://{bucket}/'
    ...
    return '\n:::END OF Customer Profile Data For Youtube Extract Load script:::'  



#loading Youtube data to snowflake using copyinto
print(sf_load())
