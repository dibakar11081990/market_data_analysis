'''
This python script picks and loads Customer Topics data from SPROUT API creates a equivalent csv file and puts it to S3
...
def customer_topics_data_extract_load():
    ...
    SCHEMA =  'SPROUT'
    TARGET_TABLE = 'CUSTOMER_TOPICS_DATA'
    ...
    if os.environ['ENVIRONMENT'] == 'prd':
        key =  'sprout/dt='+date+'/customer_topics_data.csv'
    else:
        key =  'test/dt='+date+'/customer_topics_data.csv'
    
    
    #url details
    url = 'https://api.sproutsocial.com/v1/'+customer_id+'/metadata/customer/topics'
    ...
    print("Customer Topics Data extraction started...")
    #extracting the file
    ...
    topics_df = json_normalize(topics_data['data'], 'theme_groups', ['id', 'name', 'topic_type', 'description', 'group_id', 'availability_time'], record_prefix='theme_groups_')

    # rename the columns
    topics_df.rename(columns={'id': 'topic_id', 'name': 'topic_name'}, inplace=True)

    # Expand the 'themes' column
    topics_df = topics_df.explode('theme_groups_themes')

    # Flatten the 'themes' column
    topics_df = pd.concat([topics_df.drop(['theme_groups_themes'], axis=1), topics_df['theme_groups_themes'].apply(pd.Series)], axis=1)

    # Order the column names
    topics_df = topics_df[[
    'topic_id', 'topic_name', 'description', 'topic_type', 'group_id', 'availability_time', 'theme_groups_name', 'id', 'name'
    ]]

    # Standardize the column name
    topics_df.rename(columns={'description': 'topic_description', 'group_id': 'topic_group_id', 'theme_groups_name': 'topic_theme_groups_name', 'id': 'topic_theme_groups_theme_id', 'name': 'topic_theme_groups_theme_name'}, inplace=True)

    topics_df['ingestion_timestamp'] = datetime.today()
    ...
    print(f":::Customer Topic Data extraction completed and uploaded to S3 location: {bucket}/{key}...:::")
    ...
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{SCHEMA}"."{TARGET_TABLE}"("TOPIC_ID", "TOPIC_NAME", "TOPIC_DESCRIPTION", "TOPIC_TYPE", "TOPIC_GROUP_ID", "AVAILABILITY_TIME", "TOPIC_THEME_GROUPS_NAME", "TOPIC_THEME_GROUPS_THEME_ID", "TOPIC_THEME_GROUPS_THEME_NAME", "INGESTION_TIMESTAMP")
        FROM 
        's3://{bucket}/'
...
# print(customer_topics_data_extract_load())