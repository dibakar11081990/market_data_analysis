'''
This python script picks and loads Customer tag data from SPROUT API creates a equivalent csv file and puts it to S3
...
def customer_tag_data_extract_load():
    ...
    SCHEMA =  'SPROUT'
    TARGET_TABLE = 'CUSTOMER_TAG_DATA'
    ...
    if os.environ['ENVIRONMENT'] == 'prd':
        key =  'sprout/dt='+date+'/customer_tags_data.csv'
    else:
        key =  'test/dt='+date+'/customer_tags_data.csv'
    
    
    #url details
    url = 'https://api.sproutsocial.com/v1/'+customer_id+'/metadata/customer/tags'
    ...
    print("Customer Tag Data extraction started...")
    #extracting the file
    ...
    if type(tag_data['data'][-1]) == list:
        tag_data = json_normalize(tag_data['data'][:-1])
    else:
        tag_data = json_normalize(tag_data['data'])

    tag_df = pd.DataFrame(tag_data)

    # replace empty list coming from the json to NaN
    tag_df = tag_df.mask(tag_df.applymap(str).eq('[]'))

    # remove duplicates
    tag_df = tag_df.loc[tag_df.astype(str).drop_duplicates().index]
    tag_df = tag_df.explode('groups')

    tag_df['ingestion_timestamp'] = datetime.today()
    ...
    csv_buffer = StringIO()
    tag_df.to_csv(csv_buffer, index=False)
    ...
    print(f":::Customer Tag Data extraction completed and uploaded to S3 location: {bucket}/{key}...:::")
    ...
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{SCHEMA}"."{TARGET_TABLE}"("TAG_ID", "ANY_GROUP", "TEXT", "TYPE", "GROUPS", "ACTIVE", "INGESTION_TIMESTAMP") FROM
                's3://{bucket}/'
...
# print(customer_tag_data_extract_load())