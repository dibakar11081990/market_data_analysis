'''
This python script picks and loads Customer Groups data from SPROUT API creates a equivalent csv file and puts it to S3
...
def customer_group_data_extract_load():
    ...
    SCHEMA =  'SPROUT'
    TARGET_TABLE = 'CUSTOMER_GROUP_DATA'
    ...
    if os.environ['ENVIRONMENT'] == 'prd':
        key =  'sprout/dt='+date+'/customer_groups_data.csv'
    else:
        key =  'test/dt='+date+'/customer_groups_data.csv'
    
    
    #url details
    url = 'https://api.sproutsocial.com/v1/'+customer_id+'/metadata/customer/groups'
    ...
    print("Customer Group Data extraction started...")
    #extracting the file
    ...
    if type(groups_data['data'][-1]) == list:
        groups_data = json_normalize(groups_data['data'][:-1])
    else:
        groups_data = json_normalize(groups_data['data'])

    groups_df = pd.DataFrame(groups_data)

    # replace empty list coming from the json to NaN
    groups_df = groups_df.mask(groups_df.applymap(str).eq('[]'))

    # remove duplicates
    groups_df = groups_df.loc[groups_df.astype(str).drop_duplicates().index]

    groups_df['ingestion_timestamp'] = datetime.today()
    ...
    csv_buffer = StringIO()
    groups_df.to_csv(csv_buffer, index=False)
    ...
    print(f":::Customer Group Data extraction completed and uploaded to S3 location: {bucket}/{key}...:::")
    ...
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{SCHEMA}"."{TARGET_TABLE}"("GROUP_ID", "NAME", "INGESTION_TIMESTAMP") FROM 
        's3://{bucket}/'
...
# print(customer_group_data_extract_load())