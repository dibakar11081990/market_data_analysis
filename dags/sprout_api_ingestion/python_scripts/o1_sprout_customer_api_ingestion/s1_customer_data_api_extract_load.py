'''
This python script picks and loads Customer Data from SPROUT API creates a equivalent csv file and puts it to S3
...
def customer_data_extract_load():
    ...
    if os.environ['ENVIRONMENT'] == 'prd':
        key =  'sprout/dt='+date+'/customer_data.csv'
    else:
        key =  'test/dt='+date+'/customer_data.csv' 
    
    #url details
    url = 'https://api.sproutsocial.com/v1/'+customer_id+'/metadata/customer'
    ...
    print("Customer Data extraction started...")
    #extracting the file
    ...
    if type(customer_data['data'][-1]) == list:
        customer_data = json_normalize(customer_data['data'][:-1])
    else:
        customer_data = json_normalize(customer_data['data'])

    customer_df = pd.DataFrame(customer_data)

    # replace empty list coming from the json to NaN
    customer_df = customer_df.mask(customer_df.applymap(str).eq('[]'))

    # remove duplicates
    customer_df = customer_df.loc[customer_df.astype(str).drop_duplicates().index]
    customer_df = customer_df.explode('groups')

    customer_df['ingestion_timestamp'] = datetime.today()
    ...
    print(f":::Customer Data extraction completed and uploaded to S3 location: {bucket}/{key}...:::")
    ...
    with sf_engine.begin() as connection:
        ...
        copy_into_query = f'''
        COPY INTO "{DB}"."{SCHEMA}"."{TARGET_TABLE}"("CUSTOMER_PROFILE_ID", "NETWORK_TYPE", "NAME", "NATIVE_NAME", "LINK", "NATIVE_ID", "GROUPS", "INGESTION_TIMESTAMP") FROM 
                's3://{bucket}/'
    ...
    return '\n:::END OF Customer Data Api Exctract Load script:::'
    



# print(customer_data_extract_load())